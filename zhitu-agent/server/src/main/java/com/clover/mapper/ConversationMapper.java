...
public interface ConversationMapper {
     ...
     * 插入对话记录
     */
    @Insert("INSERT INTO conversation (user_id, session_id, session_title, role, content, message_type, tool_calls) " +
            "VALUES (#{userId}, #{sessionId}, #{sessionTitle}, #{role}, #{content}, #{messageType}, #{toolCalls})")
     ...
     * 查询用户的会话列表
     */
    @Select("SELECT DISTINCT session_id, session_title, MIN(created_at) as created_at " +
            "FROM conversation WHERE user_id = #{userId} " +
            "GROUP BY session_id, session_title ORDER BY created_at DESC")
    List<SessionInfo> findSessionsByUserId(String userId);

    /**
     * 更新会话标题
     */
    @Update("UPDATE conversation SET session_title = #{sessionTitle} WHERE user_id = #{userId} AND session_id = #{sessionId}")
    int updateSessionTitle(@Param("userId") String userId, 
                           @Param("sessionId") String sessionId, 
